#!/usr/bin/env python3
"""Export and structurally validate the bounded post-INTAKE verifier return contract."""
...
QUESTIONS = (
    ("INTAKE-Q01", "Does the normalized intake preserve the exact six-field original intake?"),
    ("INTAKE-Q02", "Is the core research question falsifiable and measurable within the stated computational scope?"),
    ("INTAKE-Q03", "Are novelty and contribution statements explicitly hypotheses rather than findings?"),
    ("INTAKE-Q04", "Does the charter prevent claims stronger than the authorized evidence maturity and scope?"),
    ("INTAKE-Q05", "Is the non-human computational study classification consistent with the authorized boundary?"),
    ("INTAKE-Q06", "Are privacy, dual-use, exploitation, secret, upload, participant, intervention, and policy limits explicit?"),
    ("INTAKE-Q07", "Does the study profile resolve the required routing fields without freezing later venue rules?"),
    ("INTAKE-Q08", "Are the frozen claim boundaries and evidence-maturity ceilings internally coherent?"),
    ("INTAKE-Q09", "Is there any reason the case must remain at INTAKE after corrections?"),
)
EXPECTED_STATE = {
    "status": "ACTIVE",
...
OVERALL_DISPOSITIONS = {"PROCEED_RECOMMENDED", "REVISE", "STOP"}
...
def validate_state(state: dict[str, Any]) -> None:
    ...
    if mismatches:
        raise ContractError(
            "INTAKE return contract requires fail-closed canonical state: "
            + "; ".join(mismatches)
        )
...
def registry_artifacts(root: Path) -> list[dict[str, str]]:
    ...
    for artifact_path in ARTIFACTS:
        ...
        if row is None:
            raise ContractError(f"canonical INTAKE artifact is missing from registry: {registry_path}")
        ...
        if not actual_path.is_file():
            raise ContractError(f"canonical INTAKE artifact is missing: {artifact_path}")
...
def build_template(root: Path) -> dict[str, Any]:
    ...
    if not bundle.is_file():
        raise ContractError(f"INTAKE review packet is missing: {bundle}")
    artifacts = registry_artifacts(root)
    return {
        "schema_id": "KEYSTONE_INTAKE_VERIFIER_RETURN",
        "schema_version": 1,
        ...
        "validation_boundary": (
            ...
            "scientific correctness, independence, provenance, or phase eligibility."
        ),
    }
...
def validate_completed_return(root: Path, response: dict[str, Any]) -> None:
    ...
    if not isinstance(questions, list) or len(questions) != len(QUESTIONS):
        raise ContractError("completed return must answer all nine INTAKE questions")
    ...
    if not isinstance(artifacts, list) or len(artifacts) != len(expected_artifacts):
        raise ContractError("completed return must include exactly four artifact dispositions")
    ...
    if overall not in OVERALL_DISPOSITIONS:
        raise ContractError("overall disposition must be PROCEED_RECOMMENDED, REVISE, or STOP")
    if overall == "PROCEED_RECOMMENDED" and any(item.get("disposition") != "VERIFIED" for item in artifacts):
        raise ContractError("PROCEED_RECOMMENDED requires all four artifacts marked VERIFIED in the return")
...
def main() -> int:
    ...
    try:
        if args.validate is not None:
            ...
            print(
                "VALID STRUCTURE ONLY: return matches the current INTAKE packet contract; "
                "this does not verify signature, identity, independence, scientific correctness, provenance, or phase eligibility."
            )
            return 0
        ...
        if args.check:
            if not path.is_file() or path.read_bytes() != expected:
                raise ContractError(f"INTAKE verifier return template is stale: {path}")
            print(f"OK: INTAKE verifier return template is current: {path}")
            return 0
        if not path.is_file() or path.read_bytes() != expected:
            atomic_write(path, expected)
        print(f"WROTE: INTAKE verifier return template: {path}")
        return 0
